...
def d_step(config, vocab, model_F, model_D, optimizer_D, batch, temperature):
    ...
    if config.discriminator_method == 'Multi':
        ...
        raw_gen_log_probs = model_D(
            raw_gen_soft_tokens, raw_gen_lengths, raw_styles)
        rev_gen_log_probs = model_D(
            rev_gen_soft_tokens, rev_gen_lengths, rev_styles)
        gen_log_probs = torch.cat((raw_gen_log_probs, rev_gen_log_probs), 0)
...
def train(config, vocab, model_F, model_D, train_iters, dev_iters, test_iters):
    ...
    optimizer_F = optim.Adam(model_F.parameters(),
                             lr=config.lr_F, weight_decay=config.L2)
    optimizer_D = optim.Adam(model_D.parameters(),
                             lr=config.lr_D, weight_decay=config.L2)
    ...
    for i, batch in enumerate(bigbar):
        ...
        slf_loss, cyc_loss, _ = f_step(
            config, vocab, model_F, model_D, optimizer_F, batch, 1.0, 1.0, False)
    ...
    for _ in bigbar:
        ...
        if global_step % config.eval_steps == 0:
            ...
            # save model
            torch.save(model_F.state_dict(), config.save_folder +
                       '/ckpts/' + str(global_step) + '_F.pth')
            torch.save(model_D.state_dict(), config.save_folder +
                       '/ckpts/' + str(global_step) + '_D.pth')
            auto_eval(config, vocab, model_F, test_iters,
                      global_step, temperature)
