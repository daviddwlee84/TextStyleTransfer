...
def part2(args):
    ...
    for l in range(len(attn_weight)):
        ...
        show_attn(gold_token[idx], rev_token[idx],
                  attn_weight[l][idx], 'attention map', output_name)
    ...
    for batch in pos_iter:
        ...
        _, pos_features = model_D(
            inp_tokens, inp_lengths, return_features=True)
        ...
        _, rev_features = model_D(
            rev_tokens, inp_lengths, return_features=True)
    ...
    for i in range(4):
        idxs = labels == i
        ax.scatter(X_emb[idxs, 0], X_emb[idxs, 1], color=colors[i],
                   label=classes[i], alpha=0.8, edgecolors='none')
    ax.legend()
    ...
    # mask until '. <eos>' but contain the origin sentence
    inp_tokens = inp_token.repeat(inp_length-2+1, 1)
